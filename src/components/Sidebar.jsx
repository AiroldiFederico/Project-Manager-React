		...
		<div className="flex flex-col gap-1">
			{proj.map((row, rowIndex) => (
				<button
					...
					key={rowIndex}
					className={`text-left px-2 rounded-sm ${selectedProjectIndex === rowIndex ? 'bg-stone-700 text-white hover:bg-stone-700' : 'text-stone-400 hover:text-stone-200 hover:bg-stone-800'}`}>
					{row.title}
				</button>
			))}
		</div>
